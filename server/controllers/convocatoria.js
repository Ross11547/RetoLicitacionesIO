...
app.post("/convocatoria", async (req, res) => {
  try {
    const { nombre, descripcion, departamento, modalidad, fechaPublicacion } = req.body;
    if (!nombre || !descripcion || !departamento || !modalidad || !fechaPublicacion) {
    ...
    res.json({
      data: convocatoria,
      message: "convocatoria creado correctamente",
    });
  } catch (error) {
    res.status(500).json({
      message: "Error al agregar convocatoria",
